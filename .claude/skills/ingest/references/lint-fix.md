...
6. [domain-order] [[PPO]] имеет domain в порядке
   [Machine Learning, Reinforcement Learning] — сначала шире, потом узкий.
   Convention: от частного к общему (RL ⊂ ML).
   → переупорядочить в [Reinforcement Learning, Machine Learning] / отложить?
```
...
- "Переупорядочить" (для `domain-order`) → переписать блок `domain:` в frontmatter `where` в порядке из `expected`. Сохранить wikilink-формат (`"[[Domain Name]]"`), поменять только порядок строк
