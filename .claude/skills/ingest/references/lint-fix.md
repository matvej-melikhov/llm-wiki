...
| `raw-ref-no-alias` | добавить alias `↗` к существующему wikilink: `[[raw/X#section]]` → `[[raw/X#section\|↗]]` |
| `raw-ref-on-single-source-page` | удалить inline-маркер с тела (на single-source странице провенанс уже в frontmatter) |
...
4. [multi-source-no-citations] [[Learning to Rank]] использует 2 источника,
   но в теле нет inline-маркеров ↗.
   → расставить маркеры (потребует чтения raw для определения провенанса) / отложить?
```
...
- "Расставить маркеры" (для `multi-source-no-citations`) → прочитать каждый raw из `sources`, для каждого абзаца определить источник, добавить inline-маркеры `[[raw/X#section|↗]]` по правилам из `synthesis-phases.md` Phase 5. Убрать issue.
