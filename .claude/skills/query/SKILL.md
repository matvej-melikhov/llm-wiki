...
2. Если cache не отвечает — запустить embedding pre-filter:
   ...
   Возвращает 10 наиболее семантически близких страниц с полными путями вида `wiki/ideas/RLHF.md` (meta-файлы автоматически отфильтрованы). Saliency-порядок по cosine.

3. Прочитать топ-3-5 страниц из вывода. Если все на одну тему — возможно открыть соответствующий `wiki/domains/<X>.md` для контекста области.

4. Синтезировать ответ как в обычном Standard режиме.
...
**Эмбеддинги должны быть актуальны:** после ingest или массовой правки запусти `python3 bin/embed.py update` чтобы пересчитать изменившиеся страницы (hash-based skip — пропускает неизменённое).
