...
Lint — единственный owner всего пайплайна обработки issues. Когда команда вызвана
(пользователем напрямую или из ingest), она:
...
Layer 2 запускается **этой командой** после Layer 1+1.5. Каждая проверка ниже —
...
4. На следующем session.idle-хуке `bin/gen_index.py` подхватит саммари в
   `wiki/index.md`.
...
Применяется самим скриптом, не доходит до lint-команды. Перечислено для
контекста.
...
См. `AGENTS.md` раздел Frontmatter — схема, типы, поля. Здесь не дублируем.
