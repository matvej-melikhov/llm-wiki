...
6. **Unit-тесты для `bin/lint.py`.** Скрипт написан как тестируемый модуль (функции принимают данные, возвращают Iterable[Issue]), но набор pytest-тестов пока не создан. Планируется: `bin/tests/test_lint.py` с покрытием каждого из 13 check'ов, включая edge cases (meta pages skip, compound extensions, escaped pipes в таблицах, fenced code blocks). Для диплома важно — демонстрирует тестируемость и reproducibility.
