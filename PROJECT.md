...
5. **`sources` ссылается только на файлы из `raw/`.** Поведение в теле зависит от количества источников:
   - **Single-source страница (1 элемент в `sources`):** в теле никаких `[[raw/...]]` — провенанс уже во frontmatter.
   - **Multi-source страница (≥2 источников):** в теле inline-маркеры `[[raw/<file>#<section>|↗]]` для гранулярной атрибуции (см. §4а ниже).
...
### Цитирование raw-источников в теле (multi-source страницы)

Когда одна wiki-страница синтезирована из нескольких raw-файлов, frontmatter `sources` показывает только список — без указания, какая часть страницы из какого источника. Для верификации reviewer'ом расставляются inline-маркеры.

**Формат:** wikilink на секцию raw с alias-стрелочкой:
```markdown
[[raw/<file>#<section>|↗]]
```

Obsidian рендерит как кликабельный `↗` с hover-preview содержимого секции.

**Где ставить:**
- Маркер в конце абзаца покрывает все утверждения от предыдущего маркера до себя.
- При синтезе утверждения одного источника группируются в один абзац — это естественно и упрощает разметку.
- Конкретный факт / число / exact quote — маркер сразу после факта, независимо от абзацного правила.

**Пример:**
```markdown
## Objective

Базовый objective Ziegler 2019: $RM - \beta \cdot KL$ к SFT [[raw/RLHF#objective|↗]].
InstructGPT расширяет его третьим слагаемым — pretraining loss
с весом $\gamma$ [[raw/InstructGPT#objective-ptx|↗]].
```

**Правила:**
- Single-source страница: маркеры **не нужны** (избыточно). Lint их флагает как `raw-ref-on-single-source-page`.
- Multi-source страница без маркеров: lint флагает как `multi-source-no-citations` (ask).
- `[[raw/X.md]]` (с расширением) — auto-fix → `[[raw/X]]`.
- `[[raw/X]]` без alias `↗` — auto-fix добавляет alias.

Полные правила синтеза — `references/synthesis-phases.md` (Phase 5).
