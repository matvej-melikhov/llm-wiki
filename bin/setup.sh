...
python3 -c "import pymupdf4llm; print('  pymupdf4llm:', pymupdf4llm.__version__ if hasattr(pymupdf4llm,'__version__') else 'OK')"

echo ""
echo "Setup complete. You can now ingest:"
echo "  - Markdown files in raw/"
echo "  - URLs (defuddle)"
echo "  - PDF files (pymupdf4llm via bin/transcribe.py)"
echo "  - DOCX files (pandoc via bin/transcribe.py)"
echo "  - Images (Claude reads natively)"
